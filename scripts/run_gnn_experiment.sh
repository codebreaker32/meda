...
if [[ "$SIZE" == "16" ]]; then
  ...
  GNN_CFG=configs/training/gnn_maxpool_16x16.yaml; GNN_NAME=gnn_maxpool_16x16
else
  CNN_CFG=configs/training/paper_30x30_healthy.yaml; CNN_NAME=paper_30x30_healthy
  GNN_CFG=configs/training/gnn_maxpool_30x30.yaml; GNN_NAME=gnn_maxpool_30x30
fi
...
meda compare-methods --method "CNN-PPO" "runs/$CNN_NAME" \
                     --method "GNN-maxpool-PPO" "runs/$GNN_NAME" --episodes "$EPISODES"
